...
class TestControllerPipeline:
    ...
    def test_unstable_does_not_overwrite_status_message(self, make_controller):
        controller, ui_bridge = make_controller()
        controller._is_running = True
        ui_bridge.update_bar(controller._bar.on_csv_error("测试保留信息"))

        controller._on_raw_data("10.0 kg")  # 未稳定，不应改写 message
        assert ui_bridge._last_bar is not None
        assert ui_bridge._last_bar.message.text == "测试保留信息"
    ...
    def test_on_timeout_preserves_status_message(self, make_controller):
        ...
        assert ui_bridge._last_bar is not None
        assert ui_bridge._last_bar.message.text == MSG_ABNORMAL
        assert ui_bridge._last_bar.comm.text == "通讯等待"
    ...
    def test_abnormal_status_persists_until_recovered(self, make_controller):
        sound = MagicMock()
        controller, ui_bridge = make_controller(sound_player=sound)
        controller._is_running = True
    ...
    def test_target_status_persists_until_next_add(self, make_controller):
        sound = MagicMock()
        controller, ui_bridge = make_controller(sound_player=sound, target_pieces=2)
        controller._is_running = True
    ...
    def test_force_calibrate_waits_when_raw_stable_mismatch(self, make_controller):
        ...
        controller.request_force_calibrate(3)
        assert ui_bridge._last_bar is not None
        assert ui_bridge._last_bar.message.text == MSG_WAIT_STABLE
    ...
    def test_force_calibrate_target_edge(self, make_controller):
        sound = MagicMock()
        controller, ui_bridge = make_controller(sound_player=sound, target_pieces=3)
        controller._is_running = True
        feed_stable(controller, "10.0 kg")
        assert controller.counter_service.snapshot().total_pieces == 1
        controller.request_force_calibrate(3)
